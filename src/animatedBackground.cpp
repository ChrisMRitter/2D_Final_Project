...
void AnimatedBackground::loadSheet(std::shared_ptr<sf::Texture> sheet, float fps) {
    ...
    //Frames are horizontally arranged. Each frame has same width as height of spritesheet
...
//debugging
    ...
    if (!frames.empty()) {
        ...
        std::cerr << "Warning: No animation frames were created!" << std::endl; //debugging
    }
}
